...
n =300;   % number of random measurements obtained
S=randsample(N,n);
...
init_grid = linspace(minfreq,maxfreq,5000)';  % initial grid, uniformly spaced
...
maxiter_gp = 20;  % as of now we are not using gradient projections
tol_gp = 1e-2;
eta = 0.8;
...
for ii = 2:maxiter
    ...
    fval = evaluate_trig_pari(gradf,grid,S, 0);
    [~,f] = max(fval);
...
[coeft_final,err]=gp_reduced_complex(A_final,x,zeros(length(coeft_final),1),tau,maxiter_gp,tol_gp);
