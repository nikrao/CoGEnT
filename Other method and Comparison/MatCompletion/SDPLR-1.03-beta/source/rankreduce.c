...
{
  ptrdiff_t h, i, j, k, p, q, base, base1, base2;
  ptrdiff_t maxrank, maxblksz, newrank=0, **save;
  double tol, *U, *tempU, tv;
  ptrdiff_t info, lwork, *jpvt;
  double *work, *tau;
  ...
  MYCALLOC(save,  ptrdiff_t*,   d->numblk + 1);
  for(k = 1; k <= d->numblk; k++)
    MYCALLOC(save[k], ptrdiff_t, d->rank[k] + 1);

  lwork = mymax(1, 3*maxblksz + 1);
  MYCALLOC(work, double, lwork    + 1);
  MYCALLOC(jpvt, ptrdiff_t,    maxblksz + 1);
