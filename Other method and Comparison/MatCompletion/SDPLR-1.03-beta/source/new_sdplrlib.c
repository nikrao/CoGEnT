...
void myprint(ptrdiff_t majiter, ptrdiff_t iter, double val, double rho_f_val, ptrdiff_t CG, double totaltime)
{
  char line[1000];
  if(majiter < 0) sprintf(line, "       %7d  % .8e  %.1e  %8d  %6d",          iter, val, rho_f_val, CG, (ptrdiff_t)totaltime);
  else            sprintf(line, "  %3d  %7d  % .8e  %.1e  %8d  %6d", majiter, iter, val, rho_f_val, CG, (ptrdiff_t)totaltime);
  printf("%s",line);
  printf("\n");
  fflush(stdout);
}
