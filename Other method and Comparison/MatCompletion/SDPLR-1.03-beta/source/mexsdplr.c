...
if((VAR = (TYPE*)mxCalloc(SIZE, sizeof(TYPE))) == NULL) mexErrMsgTxt("Out of\ 
memory !");

#define MYFREE(VAR) mxFree(VAR)
...
/* extern void dsyrk_(); */
ptrdiff_t classifyApq(ptrdiff_t*, char*, ptrdiff_t*, ptrdiff_t*, ptrdiff_t, ptrdiff_t, ptrdiff_t, ptrdiff_t*, ptrdiff_t*, ptrdiff_t*, ptrdiff_t*);
...
ptrdiff_t writedata_raw(char*, ptrdiff_t, ptrdiff_t, ptrdiff_t*, char*, double*, double*, ptrdiff_t*, ptrdiff_t*, ptrdiff_t*, ptrdiff_t*, char*, char*);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  ptrdiff_t *CArow;
  ptrdiff_t *CAcol;
  ptrdiff_t *CAinfo_entptr;
  ptrdiff_t *CAinfo_rowcolptr;
  char *CAinfo_type;
  char *CAinfo_storage;
  ...
  ptrdiff_t writeraw=0;
  ptrdiff_t soln_factored=0;
  double *forcerank;
  ptrdiff_t seed;
  ...
  ptrdiff_t c_empty;
  ptrdiff_t pars_empty=1;
  ptrdiff_t lrA_empty=1;
  ptrdiff_t y0_empty=1;
  ptrdiff_t info0_empty=1;
  ptrdiff_t r0_empty=1;
  ...
  */
  ...
  mexWarnMsgTxt("I am still alive and kicking");
  ...
  if(nrhs > 3) {
    ...
    if(Ks_ != NULL) {
      ...
      for(i = 0; i < mymax( mxGetM(Ks_) , mxGetN(Ks_) ); i++)
        h += (ptrdiff_t)mat[i]*(ptrdiff_t)mat[i];
    }
    if(h != n_)
      ...
      if(nrhs > 5) {
        ...
        if(!lrA_empty) {
          ...
          for(i = 0; i < mxGetM(lrA_)*mxGetN(lrA_); i++) {
            ...
            if(field == NULL || mxGetM(field)*mxGetN(field) != 1 ||
              ...
              (ptrdiff_t)mxGetScalar(field) < 0 || (ptrdiff_t)mxGetScalar(field) > m)
              mexErrMsgTxt("sdplr: Input 6 has incorrect format.");

            field = mxGetField(lrA_,i,"start");
            if(field == NULL || mxGetM(field)*mxGetN(field) != 1 ||
              mxIsDouble(field) == 0 || mxIsComplex(field) == 1 ||
              (ptrdiff_t)mxGetScalar(field) < 0 || (ptrdiff_t)mxGetScalar(field) > n_)
              mexErrMsgTxt("sdplr: Input 6 has incorrect format.");
        ...
        if(nrhs > 6) {
          ...
          if( mxGetM(x0_) > 0 || mxGetN(x0_) > 0 )
            mexErrMsgTxt("sdplr (internal error): Input 7 has not been replaced correctly.");
  ...
  MYCALLOC(inblk, ptrdiff_t, 1);
#else
  MYCALLOC(inblk, ptrdiff_t, n_);
#endif
  MYCALLOC(inblkptr, ptrdiff_t, numblk+2);
  mexWarnMsgTxt("I am still alive and kicking");
  ...
  if(Ks_first == 1) {
    if(Ks_ != NULL) {
      ...
      for(i = 0; i < mymax( mxGetM(Ks_) , mxGetN(Ks_) ); i++) {
        inblkptr[k] = h;
        //h += (ptrdiff_t)mat[i]*(ptrdiff_t)mat[i];
        for(j = 0; j < (ptrdiff_t)mat[i]*(ptrdiff_t)mat[i]; j++) {
  ...
  if(lrA_ != NULL && !lrA_empty) {
    ...
    for(h = 0; h < mxGetM(lrA_)*mxGetN(lrA_); h++) {
      ...
      if(A_tr) i = (ptrdiff_t)mxGetScalar(field) - 1;
      else     j = (ptrdiff_t)mxGetScalar(field) - 1;

      field = mxGetField(lrA_,h,"cons");
      if(A_tr) j = (ptrdiff_t)mxGetScalar(field) - 1;
      else     i = (ptrdiff_t)mxGetScalar(field) - 1;

      classifyApq(blksz, blktype, inblk, inblkptr, i, j, A_tr, &cons, &blk, &ii, &jj);

      if(ii != 1 || jj != 1)
        mexErrMsgTxt("sdplr: Internal error (6).");
      if(cons != (ptrdiff_t)mxGetScalar(field))
        mexErrMsgTxt("sdplr: Internal error (6).");
 ...
 mexWarnMsgTxt("I am still alive and kicking");
  ...
  MYCALLOC(CArow, ptrdiff_t, nnz);
  MYCALLOC(CAcol, ptrdiff_t, nnz);
  ...
  MYCALLOC(tempint, ptrdiff_t, (m+1)*numblk);
...
/*     field = mxGetField(pars_, 0, "dir");
     if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
       method = (ptrdiff_t)mxGetScalar(field);
       */
    ...
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      rankreduce = (ptrdiff_t)mxGetScalar(field);

    field = mxGetField(pars_, 0, "limit");
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      timelim = (ptrdiff_t)mxGetScalar(field);

    field = mxGetField(pars_, 0, "printlevel");
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      printlevel = (ptrdiff_t)mxGetScalar(field);

    field = mxGetField(pars_, 0, "dthresh_dim");
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      dthresh_dim = (ptrdiff_t)mxGetScalar(field);
    ...
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      numbfgsvecs = (ptrdiff_t)mxGetScalar(field);

    field = mxGetField(pars_, 0, "auxtn");
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      doAR = (ptrdiff_t)mxGetScalar(field);
    ...
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      precond = (ptrdiff_t)mxGetScalar(field);
    ...
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      reorder = (ptrdiff_t)mxGetScalar(field);
    ...
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      typebd = (ptrdiff_t)mxGetScalar(field);

    field = mxGetField(pars_, 0, "writeraw");
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      writeraw = (ptrdiff_t)mxGetScalar(field);
    else writeraw = 0;

    field = mxGetField(pars_, 0, "soln_factored");
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      soln_factored = (ptrdiff_t)mxGetScalar(field);
    ...
    if( field != NULL && mxIsDouble(field) == 1 && mxIsComplex(field) == 0 && mxGetM(field)*mxGetN(field) == 1 )
      seed = (ptrdiff_t)mxGetScalar(field);
...
//   printf("CAinfo_entptr[0] = %d\n", CAinfo_entptr[0]);
  ...
  if(forcerank != NULL) {
    ...
    for(k = 0; k < numblk; k++) {
      ...
      if((ptrdiff_t)forcerank[k] >= 1 && (ptrdiff_t)forcerank[k] <= j)
        maxranks[k] = (ptrdiff_t)forcerank[k];
  ...
  MYCALLOC(lambda, double, m);
   ...
   mexWarnMsgTxt("I am still alive and kicking");
  srand(seed);
  for(h = 0; h < nr; h++) R[h] = (double)rand()/RAND_MAX -     
(double)rand()/RAND_MAX;
  ...
  if(!info0_empty) {
    ...
    if(field != NULL) pieces[0] = (ptrdiff_t)mxGetScalar(field);

    field = mxGetField(info0_, 0, "minoriter"); 
    if(field != NULL) pieces[1] = (ptrdiff_t)mxGetScalar(field);

    field = mxGetField(info0_, 0, "cgs"); 
    if(field != NULL) pieces[3] = (ptrdiff_t)mxGetScalar(field);
  ...
  if(nlhs > 0) {
    ...
    if(nlhs > 1) {
      ...
      if(nlhs > 2) {
        ...
        plhs[2] = mxCreateStructMatrix(1,1,6,infonames);
        if(plhs[2] == NULL) {
...
//  mxDestroyArray(c_);
//  mxDestroyArray(K_);
//  mxDestroyArray(pars_);
//  mxDestroyArray(lrA_);
//  mxDestroyArray(x0_);
//  mxDestroyArray(y0_);
//  mxDestroyArray(info0_);
//  mxDestroyArray(r0_);
//  mxDestroyArray(Kl_);
//  mxDestroyArray(Ks_);
...
ptrdiff_t classifyApq(ptrdiff_t* blksz, char* blktype, ptrdiff_t* inblk, ptrdiff_t* inblkptr, ptrdiff_t p, ptrdiff_t q, ptrdiff_t A_tr, ptrdiff_t* cons, ptrdiff_t* blk, ptrdiff_t* i, ptrdiff_t* j)
{
  ...
  *cons = r+1;
  /* SS */
#ifdef Kaushik_addition
  for(*blk = 0; inblkptr[*blk +1] <= c; (*blk)++);
#else
  *blk = inblk[c];
#endif
