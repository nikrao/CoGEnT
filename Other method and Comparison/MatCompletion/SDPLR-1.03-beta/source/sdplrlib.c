...
ptrdiff_t sdplrlib (ptrdiff_t m, ptrdiff_t numblk, ptrdiff_t *blksz, char *blktype, double *b,
              double *CAent, ptrdiff_t *CArow, ptrdiff_t *CAcol, ptrdiff_t *CAinfo_entptr,
              char *CAinfo_type, ptrdiff_t numbfgsvecs, double rho_f, double
              rho_c, double sigmafac, ptrdiff_t rankreduce, double gaptol, ptrdiff_t
              checkbd, ptrdiff_t typebd, ptrdiff_t dthresh_dim, double dthresh_dens,
              ptrdiff_t timelim, double rankredtol, ptrdiff_t printlevel, double *R,
              double *lambda, ptrdiff_t* maxranks, ptrdiff_t *ranks, double *pieces)
{
  // Paramters that are stored in 'pieces'
  ptrdiff_t    majiter, iter, CG, curr_CG, lambdaupdate;
  double timeoffset;

  // Algorithm declarations
  ptrdiff_t          lambdaupdatect, oldest;
  ...
  ptrdiff_t  i, k, localiter=0, recalc=5, recalcct=10000000;
  ...
  majiter      = (ptrdiff_t)    pieces[0];
  iter         = (ptrdiff_t)    pieces[1];
  lambdaupdate = (ptrdiff_t)    pieces[2];
  CG           = (ptrdiff_t)    pieces[3];
  curr_CG      = (ptrdiff_t)    pieces[4];
...
#endif
#ifdef __MEXSS
        mexEvalString("clear cspsd;");      // SS
        mxArray *rhs[3], *lhs[1]; 
        rhs[0] = mxCreateDoubleMatrix(data->m, 1, mxREAL);
        rhs[1] = mxCreateDoubleScalar(data->sigma);
        rhs[2] = mxCreateDoubleScalar(overallsc);
#endif
...
//         printf("data->totaltime = %f   %f\n", data->totaltime, (double)current_time(timeorig) + (double)timeoffset);
...
#ifdef __MEXSS // SS: Ensure dual feasibility
          *mxGetPr(rhs[1]) = data->sigma;
          *mxGetPr(rhs[2]) = overallsc;
          EASYDSCAL(data->m, overallsc, data->lambda);
          EASYDCOPY(data->m, data->lambda, mxGetPr(rhs[0])) ;
          mexCallMATLAB(1, lhs, 3, rhs, "cspsd");
          EASYDCOPY(data->m, mxGetPr(lhs[0]), data->lambda) ;
          EASYDSCAL(data->m, 1./overallsc, data->lambda);
#endif
            essential_calcs (data, R, normC, normb, &val, &rho_c_val, &rho_f_val);
  ...
  END_CURR_MAJOR_ITERATION:
    ...
    if(data->printlevel > 0) {
      if (data->checkbd == 1) {
        sprintf (line, "%3d %6d % .7e %.1e % .7e %5d    [ %5d  %5d ]\n", majiter, iter, val, rho_f_val, bestbd, (ptrdiff_t) data->totaltime, curr_CG, CG);
        printf ("%s",line); fflush (stdout);
      }
#ifdef Kaushik_addition
      if (data->checkbd == 0 || data->checkbd == -1) myprint(majiter, iter, 
      overallsc*val, rho_f_val, CG, data->totaltime, data->rank[1], 
      data->sigma);
#else
      if (data->checkbd == 0 || data->checkbd == -1) myprint(majiter, iter, 
      overallsc*val, rho_f_val, CG, data->totaltime);
#endif
    }
...
//     data->totaltime = current_time (timeorig) + timeoffset;
//     printf ("%.7e  %d\n", bestbd, (ptrdiff_t) data->totaltime);
//   }
...
{
  char line[1000];
  if(majiter < 0) sprintf(line, "       %7d  % .8e  %.1e  %8d  %6d  %6d %.8e", 
    iter, val, rho_f_val, CG, (ptrdiff_t)totaltime, rank, sigma);
  else sprintf(line, "  %3d  %7d  % .8e  %.1e  %8d  %6d  %6d  %.8e", majiter, 
  iter, val, rho_f_val, CG, (ptrdiff_t)totaltime, rank, sigma);
  printf("%s",line);
  printf("\n");
  fflush(stdout);
...
{
  char line[1000];
  if(majiter < 0) sprintf(line, "       %7d  % .8e  %.1e  %8d  %6d",       
  iter, val, rho_f_val, CG, (ptrdiff_t)totaltime);
  else sprintf(line, "  %3d  %7d  % .8e  %.1e  %8d  %6d", majiter, iter, val, 
  rho_f_val, CG, (ptrdiff_t)totaltime);
  printf("%s",line);
  printf("\n");
  fflush(stdout);
}
