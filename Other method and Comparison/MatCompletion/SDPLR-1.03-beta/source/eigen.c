...
ptrdiff_t my_arpack(ptrdiff_t (*matvec)(double*, double*), ptrdiff_t n, ptrdiff_t nev, ptrdiff_t ncv, char* which, ptrdiff_t maxitr, ptrdiff_t printlevel, double* evals, double* evecs, ptrdiff_t* nconv, ptrdiff_t* nummatvec)
{
  ...
  ptrdiff_t *select, iparam[11], ipntr[11];
  char bmat[2], all[4];
  ptrdiff_t ldv, ido, lworkl, info, ierr;
  ptrdiff_t mode, ishfts, rvec;
  double tol, sigma;
  double zero;
  ...
  *nummatvec = 0;
  ...
  select = (ptrdiff_t*)calloc(ncv, sizeof(ptrdiff_t));
...
{
  ptrdiff_t problem=0;
  ptrdiff_t ct, i, k, nev, maxitr, printlevel;
  char which[3];
  double eval;
  ptrdiff_t maxblksz, info, dimwork;
  char uplo, jobz;
