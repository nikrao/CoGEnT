...
Phi = randn(n,p);
beta = zeros(p,1);
a = (2*rand-1)*ones(20,1)+ randn(20,1);
b = (2*rand-1)*ones(20,1)+ randn(20,1);
c = (2*rand-1)*ones(20,1)+ randn(20,1);
d = (2*rand-1)*ones(20,1)+ randn(20,1);
...
c = cset(c);
selfun =@(gradf) find_next_atom_oscar(gradf,c);
[x, At, obj_cg, time] = CoGEnT_sparse(y, Phi, tau, Ainit, selfun, ...
    'maxiter',maxiter,...
    'eta', eta,...
    'gptol', gptol,...
    'gpiter', gpiter,...
...
for lind = 1:length(lam_range);
    ...
    for cind= 1:length(cset)
        ...
        [x_hat,~] = Adlas(Phi,y,lambda_vec,options);
...
fprintf('rerunning adlas with best params \n');
[t c] = find(err == min(min(err)));
t = t(1); c = c(1);
lambda = lam_range(t);
c = cset(c);
...
[x_hat,~,obj_adlas] = Adlas(Phi,y,lambda_vec,options);
