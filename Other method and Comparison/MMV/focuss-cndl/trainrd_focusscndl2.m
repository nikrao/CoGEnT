function [rmse, A, x, diversity, x100norm] = trainrdcol(y, x, params, Ainit, targetdiv, Aorig, xorig)
% traidrdcol - Learn a random dictionary and sparse solutions.  See
...
%           iteration.  Use the column normalization version of the learning rule
%           (A update) (see notes KKD 6/20/2001)  (FOCUSS-CNDL)
%
    ...
    for indexk = 1:N
        ...
        if(iter > 1 & mod(indexk, params.Aupdateiter) == 0 )
          ...
          %  end
            ...
            % A update used to be (in trainrd2.m):
            ...
            for i = 1:n
                ai = A(:,i);
                deltaai = deltaA(:,i);
                updateA(:, i) = gammaA*(deltaai - ai*ai'*deltaai);
            end
