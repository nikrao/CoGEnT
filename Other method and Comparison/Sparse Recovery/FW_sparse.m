function [x, At, obj, time] = FW_sparse(y, Phi, tau, Ainit, selfun , varargin)
% usage [x, At, obj, time] = FW_sparse(y, Phi, tau, Ainit, selfun , varargin)
...
% 'stopcrit'  = default = 0; stopping criterion. 0 = obj<tol. 1 = relative
%               change in obj < tol
%
% OUTPUTS :
% x           = final result
...
while iter < maxiter
    ...
    if classic==0
        ...
        iter
        cvx_begin quiet
        variable z(size(Wt,2),1)
        minimize sum_square(y - Wt*z)
        subject to
        sum(abs(z))<=tau;
        cvx_end
