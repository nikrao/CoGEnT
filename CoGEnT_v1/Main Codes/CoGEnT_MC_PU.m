function [x, At, iter, obj, time, back_count] = CoGEnT_MC_PU(y, Phi, tau, Ainit, matsize ,selfun, a ,varargin)
% usage [x, At, iter, obj, time, back_count] = CoGEnT_MC(y, Phi, tau, Ainit, matsize ,selfun, varargin)
%
...
y = sparse(y);
indpos = find(y==1);
indneg = setdiff(1:size(Phi,1),indpos);
y1 = y(indpos); y0 = y(indneg);
P1 = Phi(indpos,:); P0 = Phi(indneg,:);
y = [a*y1; (1-a)*y0];
Phi = [a*P1; (1-a)*P0];
clear y1 y0 P1 P0
while iter <= maxiter
