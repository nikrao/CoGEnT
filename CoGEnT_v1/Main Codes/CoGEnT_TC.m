function [x, At, iter, obj, time, back_count] = CoGEnT_TC(y, Phi, tau, Ainit, tensize ,selfun, varargin)
% usage [x, At, iter, obj, time, back_count] = CoGEnT_TC(y, Phi, tau, Ainit, matsize ,selfun, varargin)
%
% FUNCTION PERFORMS A MINIMIZATION OF THE FORM:
% min_x 0.5|| y - Phi x ||^2  s.t ||x||_T* \leq tau
% where ||x||_T* is the symmetric tensor rank as defined in
% Anadkumar et. al, "Tensor decompositions for learning latent variable
% models" arXiv preprint 1210.7559
%
%
% INPUTS:
%
...
% tensize     = vector of dimensions of tensor
%
% OPTIONAL INPUTS
...
% At, coeft need to be preallocated
...
while iter <= maxiter
    ...
    if backward
        ...
        if ~isempty(S)
            numats = length(S);
            A_new = zeros(n1*n2*n3,numats);
            for ats = 1:numats
                a = ktensor(1,U(:,ats),U(:,ats),U(:,ats));
                a = full(a);
                A_new(:,ats) = a(:)/norm(a(:));
            end
