function [x1,x2,At1, At2, iter, obj, time, back_count] = CoGEnT_Demix2(y, Phi, tau1, tau2,...
    Ainit1, Ainit2, selfun1, selfun2, matsize,varargin)
...
%     x1=x1+alpha1*(tau1*next_atom-x1);
    ...
    for loop=1:mult_back
        ...
        clear next_atom A_new x_rem coeft_new;
        %end
    end
    %clear next_atom;
   ...
   % alpha2=linesearch(y-x1-x2,tau2*next_atom-x2); %1 D line search alpha \in [0,1]
    ...
    for loop2=1:mult_back
        ...
        if ((obj_back - obj_fwd < (obj(iter-1)-obj_fwd)*eta))
            ...
            back_count = back_count +1;
            if verbose
                fprintf('#backward steps atom 2 = %d, iterations = %d \n', back_count,iter);
            end
            obj(iter)=obj_back;
        else
            if loop2==1
