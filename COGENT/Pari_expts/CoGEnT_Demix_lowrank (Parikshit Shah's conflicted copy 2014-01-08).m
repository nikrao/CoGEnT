...
while iter <= maxiter
    ...
    At1(:,currlength1) = next_atom; % update the atomic set
    
    % conditional gradient step
    phinext=Phi*(tau1*next_atom-(x1+x2));
%     phinext=Phi*(tau1*next_atom-(x1));
    gamma=-(resid'*phinext)/(phinext'*phinext);
    gamma= median([0,gamma,1]);
    coeft1=(1-gamma)*coeft1;
    coeft1(currlength1)=gamma*tau1;
    
    if gp_forward==1
        [coeft1(1:currlength1),~]=grad_proj(Phi*At1(:,1:currlength1),y,coeft1(1:currlength1),tau1,gpiter,gptol);
    end
    
    % update iterate, and store the atoms and coefficients
    x1 = At1(:,1:currlength1)*coeft1(1:currlength1);
    history1(:,currlength1) = Phi*next_atom;
    
    obj_fwd = 0.5 * norm(Phi*(x1+x2)-y)^2;
    ...
    if backward
        ...
        while do_multiple
            ...
            [coeft_new,~]=grad_proj(Phi*A_new,y,coeft_new,tau1,gpiter,gptol);
            ...
            if ((obj_back - obj_fwd < (obj(iter-1)-obj_fwd)*eta))
                % backward step taken
                currlength1 = currlength1-1;
                At1(:,1:currlength1) = A_new;
                x1 = x_rem;
                coeft1(1:currlength1) = coeft_new;
                history1(:,1:currlength1) = Hnew;
                obj(iter) = obj_back;
                time(iter) = toc;
    ...
    At2(:,currlength2) = next_atom; % update the atomic set
    
    % conditional gradient step
    phinext=Phi*(tau2*next_atom-(x1+x2));
%     phinext=Phi*(tau2*next_atom-(x2));
    gamma=-(resid'*phinext)/(phinext'*phinext);
    gamma= median([0,1,gamma]);
    coeft2=(1-gamma)*coeft2;
    coeft2(currlength2)=gamma*tau2;
    
    if gp_forward==1
        [coeft2(1:currlength2),~]=grad_proj(Phi*At2(:,1:currlength2),y,coeft2(1:currlength2),tau2,gpiter,gptol);
    end
    
    % update iterate, and store the atoms and coefficients
    x2 = At2(:,1:currlength2)*coeft2(1:currlength2);
    ...
    if backward
        ...
        [coeft_new,~]=grad_proj(Phi*A_new,y,zeros(numats,1),tau2,gpiter,gptol);
        x_rem = A_new*coeft_new;
        obj_back = 0.5* norm(y - Phi*(x1+x_rem))^2;
        if ((obj_back - obj_fwd < (obj(iter-1)-obj_fwd)*eta))
